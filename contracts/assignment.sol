...
contract ProposalContract {
    ...
    function calculateCurrentState() private view returns(bool) {
        ...
        // Changed the logic for calculating proposal state
        if (approve > reject && approve > pass) {
            return true; // Proposal passes if approves are the highest
        } else if (reject > approve && reject > pass) {
            return false; // Proposal fails if rejects are the highest
        } else {
            return pass >= approve && pass >= reject; // Proposal passes if passes are equal or higher than approves and rejects
        }
    }
